...
- Repeatables set `immediately: false` so the first run waits a full interval (startup still calls `syncAccount()` once). Completed/failed jobs keep the last 20/50 records.
- `execution-reconcile` updates only rows still `PENDING`/`AMBIGUOUS`. A close without `positionId` is skipped (never treated as an open).
