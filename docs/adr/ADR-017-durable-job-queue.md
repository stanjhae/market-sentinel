...
- Repeatables set `immediately: false` so the first run waits a full interval (startup still calls `syncAccount()` once). Completed/failed jobs keep the last 20/50 records. Worker `lockDuration` is 10 minutes so a slow REST account sync is not stalled.
